...
export const PageBanner: React.FC<PageBannerProps> = ({
    ...
    return (
        <div className={`relative text-left h-[250px] sm:h-[300px] md:h-[300px] lg:h-[300px] ${ imageSrc ? "" : "bg-gray-100 dark:bg-black"}`}>            
                {imageSrc ? (
                    <Image src={imageSrc} alt={`${title} Cover`} className="block h-full w-full object-cover" fill priority/>
                ): (
                ...
                <hr className="absolute bottom-0 w-full h-px bg-gray-300 border-0 dark:bg-gray-800" />
            </div>             
    );
};